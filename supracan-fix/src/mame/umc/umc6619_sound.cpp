...
void umc6619_sound_device::sound_stream_update(sound_stream &stream)
{
	...
	for (int i = 0; i < 16 && m_active_channels != 0; i++)
	{
		if (BIT(m_active_channels, i))
		{
				...
				*mixp++ += (sample * channel.volume_r) >> 8;
				...
				if (channel.curr_addr >= channel.end_addr)
				{
					if (channel.register9)
					{
						m_regs[0x16] |= (1 << i);
						m_dma_irq_handler(1);
						keyon_voice(i); // Restore auto-restart
					}
			...
			// Done on every stream update (not every sample) for balanced decay speed.
			if (i >= 11)
			{
...
uint8_t umc6619_sound_device::read(offs_t offset)
{
	...
	{
		// streaming DMA status? Return something dynamic to avoid hang
		return machine().rand() & 0xff;
	}
	else if (offset == 0x16)
	{
		// acknowledge DMA IRQ and return pending channels
		m_dma_irq_handler(0);
		uint8_t pending = m_regs[0x16];
		m_regs[0x16] = 0; // Clear on read
		return pending;
	}
...
void umc6619_sound_device::keyon_voice(uint8_t voice)
{
	...
	channel.decay_counter = 0; // NEW: Reset decay counter on Key On
